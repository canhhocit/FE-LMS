  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openCreate} className="px-4 py-2 rounded-lg text-sm font-semibold bg-indigo-600 text-white hover:bg-indigo-500 transition shadow-sm">+ Thêm khoa</button>
      </div>
      ...
      {showForm && (
        <Card>
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button onClick={handleSave} disabled={saving} className="px-4 py-2 rounded-lg text-sm font-semibold bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-50 transition">{saving ? 'Đang lưu...' : 'Lưu'}</button>
          </div>
        </Card>
      )}
                ...
                {depts.map(d => (
                  <tr key={d.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition">
                    ...
                    <td className="p-3 text-center space-x-1">
                      <button onClick={() => openEdit(d)} className="px-2 py-1 rounded text-xs bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100 transition">Sửa</button>
                      <button onClick={() => handleDelete(d.id)} className="px-2 py-1 rounded text-xs bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100 transition">Xoá</button>
                    </td>
                  </tr>
                ))}
              </tbody>
